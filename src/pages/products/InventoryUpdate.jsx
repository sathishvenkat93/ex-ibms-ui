...
export default function InventoryUpdate() {
  ...
  const fetchInventoryEndpoint = `http://localhost:3000/offline/v1/inventory/${productId}`;
  const updateInventoryEndpoint = `http://localhost:3000/offline/v1/inventory/update/${productId}`;
  ...
  useEffect(() => {
    ...
    const fetchInventoryData = async () => {
      try {
        const response = await fetch(fetchInventoryEndpoint);
        const data = await response.json();
        ...
        setPrintRequired(inventory.printrequired);
        setPrintDetails(inventory.printDetails);
        setSelectedSkus(inventory.SKU);
        const unitMap = inventory.SKU.reduce((acc, sku) => {
          acc[sku.modelId] = sku.units;
          return acc;
        }, {});
        setUnits(unitMap);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    const skuData = selectedSkus
      .filter((sku) => units[sku.modelId] > 0)
      .map((sku) => ({
        modelId: sku.modelId,
        units: units[sku.modelId],
      }));

    if (skuData.length !== selectedSkus.length) {
      handleValidationError('Units must not be zero, negative, or null.');
      return;
    }
    ...
    try {
      const response = await fetch(updateInventoryEndpoint, {
        method: 'POST',
        headers: {
  ...
  const handleSkuSelection = (modelId) => {
    setSelectedSkus((prevSelectedSkus) =>
      prevSelectedSkus.includes(modelId)
        ? prevSelectedSkus.filter((id) => id !== modelId)
        : [...prevSelectedSkus, modelId]
    );
  };
